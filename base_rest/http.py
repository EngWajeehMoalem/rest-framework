...
def wrapJsonException(exception, include_description=False, extra_info=None, errors=None):
  ...
  exception.code = 422 if errors else exception.code
  ...
  if request:
    ...
    message = ("RESTFULL call to url %s with method %s and params %s "
               "raise the following error %s")
    params = (request.params.copy()
              if hasattr(request, "params") else request.get_http_params().copy())
...
class RestApiDispatcher(Dispatcher):
  ...
  def pre_dispatch(self, rule, args):
    ...
    if httprequest.mimetype == "application/json":
      ...
      self.request.params.update(pyquerystring.parse(httprequest.query_string.decode("utf-8")))
  ...
  def handle_error(self, exception):
    ...
    if isinstance(exception, (UserError, ValidationError)):
      ...
      return wrapJsonException(BadRequest(exception.args[0]),
